...
def get_dataset(
    ...
    if os.path.exists(path) and not rebuild:
        ...
        if name == "cnn":
            ...
            build_fn = build_wikiann
        elif name == "conll2003":
            build_fn = build_conll
        else:
...
def build_conll(
    ...
    """
    Build CoNLL-2003 dataset with frozen encoder embeddings.
    Each example is a sentence with NER tags.
    """
    ds = load_dataset("conll2003", split=split)  # HuggingFace CoNLL-2003
    if subset is not None:
        if subset <= 1.0:
            subset = int(len(ds) * subset)
        ds = ds.select(range(subset))
    ...
    def _tokenize_and_encode(x):
        # CoNLL provides tokens directly
        text = " ".join(x["tokens"])
        enc = tok(text, truncation=True, max_length=max_length, is_split_into_words=False)
        inputs = {
            "input_ids": torch.tensor(enc["input_ids"]).unsqueeze(0),
