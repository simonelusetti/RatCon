...
def random_selector(p: float) -> Callable:
    @torch.no_grad()
    def _selector(_, attn):
...
@torch.no_grad()
def encode_with_optional_gates(
    encoder: SentenceEncoder,
    ids: torch.Tensor,
    attn: torch.Tensor,
    gates: torch.Tensor | None,
):
    masked_attn = attn if gates is None else attn * gates

    if isinstance(encoder, FrozenLLMEncoder):
        return encoder.encode(ids, masked_attn, original_attn=attn)
    else:
        return encoder.encode(ids, masked_attn)


@torch.no_grad()
def evaluate_retrieval(
    encoder: SentenceEncoder,
    selector: RationaleSelectorModel | Callable | None,
    ...
    for batch in tqdm(dataloader, desc="Retrieval eval"):
        ...
        if selector is None:
            q_emb = encode_with_optional_gates(encoder, q_ids, q_attn, None)
            d_emb = encode_with_optional_gates(encoder, d_ids, d_attn, None)
        else:
            q_g = compute_gates(encoder, selector, q_ids, q_attn)
            d_g = compute_gates(encoder, selector, d_ids, d_attn)

            q_emb = encode_with_optional_gates(encoder, q_ids, q_attn, q_g)
            d_emb = encode_with_optional_gates(encoder, d_ids, d_attn, d_g)
...
def main(cfg: DictConfig):
    ...
    # Infer model dim exactly like training
    with torch.no_grad():
        sample = dataset[0]
    ...
    for seed in cfg.eval.random_seeds:
        ...
        rand_sel = random_selector(p_sel)
        ranks = evaluate_retrieval(encoder, rand_sel, loader, device)
